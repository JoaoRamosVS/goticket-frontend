...
type MyEventsTableProps = {
    ...
    filteredEvents: OrganizerEventListItemDTO[];
    page: number;
    totalPages: number;
...
const EventRow = ({ event, onEdit }: EventRowProps) => {
    ...
    const location = [event.venueName, event.venueCity, event.venueState]
        ...
        .join(", ");
    const formattedDate = formatEventDate(event.startDate ?? undefined);
    const statusVisual = getStatusVisual(event.statusName);
        ...
        <tr className="border-b border-white/70 hover:bg-primary/5 hover:scale-[1.02] overflow-hidden transition-all duration-200">
            ...
            <td className="py-3 pr-4 align-middle">
                <span
                    ...
                    style={{
                        background: statusVisual.gradient,
                        boxShadow:
                            "0 4px 12px -3px rgba(0,46,71,0.25), inset 0 1px 0 0 rgba(255,255,255,0.35)",
                    }}
                >
